...
def parserefs(refs):
    ...
    req = requests.post(url, headers={'Content-Type': 'application/json;charset=UTF-8'},
                        data=postdata.encode('utf8')).text
...
def pattcomp(f1, f2, method='lccs'):
    ...
    return {'filename1': orda['filename'], 'filename2': ordb['filename'],
            'orders': zip_longest(orda['order'], ordb['order']), 'comp': res}
